...
| ------ | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GAP-1  | Influencers, follows, shoppable posts, stories                     | Nothing                                                                                                      | **Still open.** The only feature with no contract at all. Built in Phase 11 behind `InfluencerRepository`, bound to `MockInfluencerRepository`; the routes are specified for the backend in `INVENTED_ENDPOINTS.md` (FA1). Stories are not built — the prototype has no story screen. |
...
- Where the API has no contract (GAP-1 only, since the expanded collection landed), the repository is still defined by a domain port;
  ...
  Where a resource is contracted for reading but not for a write the UI needs, the repository keeps
  its `Http` name and the provisional call is confined to one data-source method — as
  `NotificationRemoteDataSource.markAllRead` is.
...
| `POST /api/v1/wallet/charge`                | custom handler                | Contracted (D19); enveloped with `gateway`/`referenceId` as the collection's own script expects, plus the `gatewayOrderId` addition |
| `GET /api/v1/payments/paymob/checkout/:id`  | custom handler                | A stand-in hosted page, so `paymentUrl` leads somewhere a browser can open                                                          |
| `GET /api/v1/payments/paymob/wallet-return` | custom handler                | Contracted; settles the charge and redirects to `brandhub://payment/result`                                                         |
...
| S10 | Screen protection     | `FLAG_SECURE` on Android for the wallet, gifts and payment-result screens, applied by `useScreenProtection` in the composition root. iOS has no equivalent flag; blurring the app switcher there is not built (§33 item 23). |
...
| AD-26 | **`expo-web-browser` and `expo-screen-capture`** for the hosted payment and wallet screen protection                                                   | A `WebView` inside the app; no screen protection      | The top-up must never see card details in BRANDHUB's own process (§28 S6), and `openAuthSessionAsync` both hosts the page and catches the return. `expo-screen-capture` is `FLAG_SECURE` behind a hook, which §28 S10 requires. | Architectural      |
...
13. **An address does not yet resolve to a shipping area by contract** (D13, amended). The app
    matches the address city against the area name and then, failing that, the governorate. Since
    Phase 9 the address form's city select is driven by `/areas`, so an address the app itself saved
    always matches by name and its delivery price is exact; the governorate arm only covers records
    created elsewhere. It is contained in one function, `resolveAddressArea`, which returns an
    explicit `areaId` unchanged the day the backend supplies one.
...
17. **Wallet-to-wallet transfers exist in the API but not in the UI.** `/wallet/transfers` is a
    ...
    never shows it, so Phase 10 built gift money and left transfers alone. Worth raising as a
    product question rather than assuming it was an oversight.
18. **Notifications have no push delivery** (D17), so the list only updates when the app is opened
    or refreshed. Marking one read has no contract either; Phase 11 invents `POST
/notifications/read-all` for the prototype's mark-all-read action (FA1).
...
21. **The contract never hands the client the gateway order id it needs to poll a payment.**
    `GET /payments/PAYMOB/status?orderId=` is contracted, but the collection sets
    `PAYMOBGatewayOrderId` by hand from the PAYMOB portal. The mock returns it on the charge and
    the backend is asked to do the same; the fallback, already implemented, is the `order`
    parameter on the gateway's return URL.
22. **Top-up limits are the client's until the backend publishes them.** The API has limits for
    wallet transfers and none for top-up, so `TOP_UP_MINIMUM` and `TOP_UP_MAXIMUM` match the
    transfer figures rather than inventing a second scale.
23. **iOS has no screenshot protection.** §28 S10's `FLAG_SECURE` is Android-only;
    `expo-screen-capture` is a no-op on iOS, and blurring the app switcher there is not built.
24. **Only one ticket category is evidenced by the contract.** The collection's single example
...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FA1**     | Support tickets, returns, gift money, the delivery OTP and stock are **delivered** in the expanded collection. What remains: **influencers, shoppable posts and follows** (specified in full by Phase 11), **marking a notification read** (Phase 11), **delivery time slots and the express flag**, **phone OTP sign-in**, confirming how an address resolves to a shipping area, and **a real field for the address label** (Phase 9). | Backend team  | Mock-only repository implementations behind real domain ports, named to make their provisional status obvious. | Only the social-commerce feature stops at migration. Everything else now runs on the real contract; notifications keep reading, and only mark-all-read goes dark. |
...
address `city` against `area.name` and, failing that, `area.governorate` — name first across the
whole list, because "Muscat" is both an area and Seeb's governorate and a field-by-field scan would
hand a Muscat address to Seeb's price. Phase 9 narrowed the gap further by driving the address
form's city select from `/areas`, so an address the app saved always matches by name. The whole
workaround is one function, `resolveAddressArea`, shared by checkout and the address slice.
Confirming the real linkage is folded into FA1.

**The address label has the same shape of problem.** The API has no field for the prototype's
HOME/WORK/OTHER label, and D13 says nothing the UI collects may be lost. The mapper carries it in
`addressLine2` behind a `brandhub-label:` marker, reading any other text in that field back as part
of `details` so a seeded record round-trips its content. It is two functions in
`data/addresses/mappers`, and it also folds into FA1.
